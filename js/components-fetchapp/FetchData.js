...
import { state, createStateObject, createCategoriesObject, createTagsObject } from "./State.js"
...
export async function loadResults() {
	try {
		const { posts, categories, tags } = await fetchPosts("posts", "categories", "tags")
		...
		state.pagination.page = localStorage.getItem("page") ? +localStorage.getItem("page") : 1
	} catch (err) {
		throw err
	}
}
export async function loadSearchResults(query) {
	try {
		const { posts, categories, tags } = await fetchPosts("posts?search=" + query, "categories", "tags")
		state.results.posts = createStateObject(posts, categories, tags)
		state.sortCategories = createCategoriesObject(categories)

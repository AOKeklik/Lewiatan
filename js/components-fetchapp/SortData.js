...
export function sortDataByTag() {
    ...
    sortedData =
        states.state.sortTags.currentSort === null
            ? states.state.data.slice()
            : states.state.data.slice().filter((el) => {
                  return el.tags.some((n) => {
                      return n.slug === states.state.sortTags.currentTag
                  })
              })

    states.state.data = sortedData
}
export function sortDataByPagination(
    page = states.state.pagination.page,
    dataOnly = null
) {

...
export function createTagsObject(tags) {
    ...
    // newData.currentTag = window.location.pathname
    //     .split("/")
    //     .filter(Boolean)
    //     .pop()
    newData.currentTag = "owoce-i-warzywa"
...
export function saveStateObjectByCategory(catID = null) {
    ...
    state.sortCategories.currentCategory = state.sortCategories.categories.find(
        (e) => {
            return e.id === theID
        }
    )
...
export function saveStateObjectByTag() {
